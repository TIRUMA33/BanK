...
public interface TransaccionRepository extends JpaRepository<TransaccionEntity, Integer> {
    ...
    @Query("select t from TransaccionEntity t where (t.cuentaBancoByCuentaDestino.id=:cuenta and t.cuentaBancoByCuentaOrigen.id=:cuentaFiltro) or (t.cuentaBancoByCuentaOrigen.id=:cuenta and t.cuentaBancoByCuentaDestino.id=:cuentaFiltro) order by t.fechaInstruccion,t.fechaEjecucion,t.cantidad")
